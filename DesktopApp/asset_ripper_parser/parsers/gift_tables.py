...
class GiftTable:
    ...
    def __str__(self):
        ...
        result.append(
            f"|loveGroups    = [[:Category:Universally_Loved_Gifts|Universally Loved Items]]"
        )

        result.append(f"|likeResponse  = {', '.join(self.liked_responses)}")
        result.append(f"|like          = {', '.join(self.liked_items)}")
        result.append(
            f"|likeGroups    = [[:Category:Universally_Liked_Gifts|Universally Liked Items]]"
        )
        ...
        result.append(
            f"|dislikeGroups = [[:Category:Universally_Disliked_Gifts|Universally Disliked Items]]"
        )
        ...
        try:
            ...
            for gift_level in list(GiftLevel):
                try:
                    ...
                    logging.error(
                        f"Could not get {gift_level.value} gifts for {path}",
                        exc_info=True,
                    )
                    pprint.pprint(main_component)
            ...
            for i in range(0, len(main_component["birthdayResponses"])):
                if i == 0:
                    gift_table.disliked_birthday_response = main_component[
                        "birthdayResponses"
                    ][i]
                elif i == 1:
                    gift_table.liked_birthday_response = main_component[
                        "birthdayResponses"
                    ][i]
                elif i == 2:
                    gift_table.loved_birthday_response = main_component[
                        "birthdayResponses"
                    ][i]
