...
class FileIndexer:
    def __init__(self, assets_folder, ids_file, file_tags_file):
        ...
        self.folders_to_index = ['MonoBehaviour', 'Resources', 'PrefabInstance'] # maybe add 'Sprite'?
        ...
        with open(self.ids_file, 'r') as ids_file:
            while True:
                ...
                if f",{guid}" in line:
                    ...
                    return re.sub(r"[0-9]+ - (.+)\.asset", "\\1", dirty_name).replace(".asset", "")
    ...
    def _map_guids(self, report_progress=None):
        ...
        for path, subdirs, files in os.walk(self.assets_folder):
            for name in files:
                ...
                is_sprites_file = "Sprite" in path or ('icon' in name and '.asset' in name)
                is_texture_file = ".png" in name
                is_data_file = [x for x in self.folders_to_index if x in path]
                if not is_data_file and not is_sprites_file:
        ...
        with open(self.file_tags_file, 'w') as file_tags_file:
            ...
            for subdir in subdirs:
                ...
                for filename in files:
                    total_processed += 1
                    if total_processed % progress_increment == 0 and report_progress is not None:
                        report_progress()
                    ...
                    if is_unparseable_file or self.ignore_file(filename) or "_0" in filename:
                        continue
                    ...
                    try:
                        ...
                        if re.match(r"[0-9]{3,5} - .+[^(_0)]\.asset", filename) is not None:
                        ...
                        if '_placeable.prefab' in filename:
                        ...
                        logging.error(f"Unable to open {filename} at {subdir}", exc_info=True)
